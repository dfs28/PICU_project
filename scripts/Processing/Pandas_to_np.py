...
parser.add_argument("-l", "--length", default=180, type=int, help="Length of input window in minutes")
...
print(f'Running with {round(input_length/60)}h time input window, no z-scores: {no_z}')
...
if args['no_z'] == False:
    point_cols = ['ALT', 'Albumin', 'AlkPhos','AST', 'Aspartate', 'Amylase', 'APTT', 'Anion_gap', 'Base_excess', 'Basophils', 'Bicarb',
                ...
                #'ethnicity', 
                'Weight_z_scores', 'Height_z_scores'] #?Take out ethinicity?
...
#Now run it
file_start = '/store/DAMTP/dfs28/PICU_data/np_arrays'
...
array3d, array2d, array_characteristics, splines, outcomes, slopes, intercepts, r_values, p_values, std_errs, slicesPerPatient, all_used_cols, j_point_cols, j_series_cols = make_3d_array(flowsheet, input_length, all_cols, point_cols, series_cols)
