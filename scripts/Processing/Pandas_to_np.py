...
def make_3d_array(array, length, all_cols, point_cols, series_cols, percentile = True):
    ...
    for position, i in enumerate(to_use):
        ...
        all_SBP = array.loc[next_slices, 'SBP_zscore']
        all_inotropes = array.loc[next_slices, 'Inotropes_kg']
        all_lactate = array.loc[next_slices, 'Lactate']
        
        #Get maximum PEWS from current slice
        min_SBP = np.min(array.loc[start_position:end_position, 'SBP_zscore'])
        max_inotropes = np.max(array.loc[start_position:end_position, 'Inotropes_kg'])
        max_lactate = np.max(array.loc[start_position:end_position, 'Lactate'])

        #Get all PEWS deterioration
        worse_SBP = np.logical_and(all_SBP < min_SBP, all_SBP < -1)
        worse_inotropes = all_inotropes > max_inotropes
        worse_lactate = np.logical_and(all_lactate > max_lactate, all_lactate > 2)
        all_worse = reduce(np.logical_or, (worse_SBP, worse_inotropes, worse_lactate))
        next_dates = all_dates[all_worse.index]
        worse_date = next_dates[all_worse]

        if len(worse_date) > 0 or array.loc[end_position, 'time_to_death'] <= 1/365:
            
            #Need this clause otherwsie time_to_deteriorate messed up if low next pews but dies
            if len(worse_date) > 0:
                ...
                time_to_deteriorate = pd.Timedelta(array.loc[end_position, 'time_to_death']*365, 'D') 
            
            #Currently setting cutoffs to <6h, 6-24h, >24h
            if time_to_deteriorate < np.timedelta64(6, 'h') or array.loc[end_position, 'time_to_death'] <= 0.25/365:
                outcomes[position, 8] = 1
            elif time_to_deteriorate < np.timedelta64(1, 'D') or array.loc[end_position, 'time_to_death'] <= 1/365:
...
np.savez('/store/DAMTP/dfs28/PICU_data/np_arrays.npz', d3 = array3d, d2 = array2d, chars = array_characteristics, splines = splines, slopes = slopes, intercepts =  intercepts, r_values = r_values, p_values = p_values, std_errs = std_errs,
