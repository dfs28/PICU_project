### Script for hyperparameter training of depthwise causal dilated 1d net
...
def TPC(hp):
    ...
    for i in range(hp.Int('num_layers', 1, 3)):
        x = tf.keras.layers.SeparableConv1D(hp.Int("num_filters1" + str(i), min_value=10, max_value=50, step=5, default = 50), 
                      hp.Int("kernel_size1" + str(i), min_value=5, max_value=60, step=5, default = 32), activation='relu', padding = 'causal',
        ...
        x = tf.keras.layers.SeparableConv1D(hp.Int("num_filters2" + str(i), min_value=10, max_value=50, step=5, default = 50), 
                      hp.Int("kernel_size2" + str(i), min_value=5, max_value=60, step=5, default = 32), activation='relu', padding = 'causal',
                      depthwise_regularizer= depthwise_regulariser,
                      pointwise_regularizer= pointwise_regulariser,
                      bias_regularizer = bias_regulariser,
                      dilation_rate=i + 1, 
                      data_format = 'channels_first')(x)
        x = layers.MaxPooling1D(hp.Int('pool_size1', min_value = 1, max_value = 7), padding = 'same')(x)
...
best_model.save('/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TPC2_causal')

#Save the best hyperparameters
best_hyperparameters = tuner.get_best_hyperparameters(1)[0]

a_file = open("/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TPC2_causal.json", "w")
