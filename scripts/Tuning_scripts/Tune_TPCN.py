...
def temporal_convolutional_pointwise_net(hp):
    ...
    z = layers.Conv1D(hp.Int("layer_size0", min_value=10, max_value=50, step=5, default = 50), 
    ...
    z = layers.Conv1D(hp.Int("layer_size1", min_value=10, max_value=50, step=5, default = 50), 
    ...
    z = layers.MaxPooling1D(hp.Int('pool_size1', min_value = 1, max_value = 7), padding = 'same')(z)

    z = layers.Conv1D(hp.Int("layer_size2",min_value=5, max_value=50, step=5, default = 50), 
                      hp.Int("filter_size2", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser)(z)

    z = layers.Conv1D(hp.Int("layer_size3", min_value=5, max_value=50, step=5, default = 50), 
                      hp.Int("filter_size3", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser)(z)

    z = layers.MaxPooling1D(hp.Int('pool_size2', min_value = 1, max_value = 7), padding = 'same')(z)
...
tuner = MyTuner(
    temporal_convolutional_pointwise_net,
    objective="val_loss",
    max_epochs=30,
...
#Do the search
tuner.search(
    [train_array3d, train_transpose, train_array2d], [train_outcomes[:, 2:5], train_outcomes[:, 5:8], train_outcomes[:, 8:11]],
