...
def TCN_lstm(hp):
    ...
    if hp.Choice('3rd block', ['True', 'False']):
        ...
        x = layers.MaxPooling1D(hp.Int('last pooling layer', min_value = 1, max_value = 4), padding = 'same')(x)
    ...
    if hp.Choice('bidirectional_init', ['True', 'False']):
        x = tf.keras.layers.Bidirectional(layers.LSTM(hp.Int('layer_size1', min_value = 10, max_value = 150, step = 20), return_sequences = (hp.Choice('2 layer lstm', ['True', 'False']) or return_sequences)))(x)
    else:
        x = layers.LSTM(hp.Int('layer_size1', min_value = 10, max_value = 150, step = 20), return_sequences = (hp.Choice('2 layer lstm', ['True', 'False']) or return_sequences))(x)

    if hp.Choice('2 layer lstm', ['True', 'False']):
        if hp.Choice('bidirectional1', ['True', 'False']):
            x = tf.keras.layers.Bidirectional(layers.LSTM(hp.Int('layer_size2', min_value = 10, max_value = 150, step = 20), return_sequences= hp.Choice('2 layer lstm', ['True', 'False'])))(x)
        else:
            x = layers.LSTM(hp.Int('layer_size2', min_value = 10, max_value = 150, step = 20), return_sequences= hp.Choice('2 layer lstm', ['True', 'False']))(x)
...
best_model.save('/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TCN_lstm2')

#Save the best hyperparameters
best_hyperparameters = tuner.get_best_hyperparameters(1)[0]

a_file = open("/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TCN_lstm2.json", "w")
