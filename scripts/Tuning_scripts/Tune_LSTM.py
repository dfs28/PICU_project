...
def lstm(hp):
    ...
    if hp.Choice('bidirectional_init', ['True', 'False']):
        x = tf.keras.layers.Bidirectional(layers.LSTM(hp.Int('layer_size1', min_value = 40, max_value = 150, step = 20), return_sequences= (num_lstm > 0)))(input_timeseries)
    else:
        x = layers.LSTM(hp.Int('layer_size1', min_value = 40, max_value = 150, step = 20), return_sequences= (num_lstm > 0))(input_timeseries)

    for i in range(num_lstm):
        if hp.Choice('bidirectional' + str(i), ['True', 'False']):
            x = tf.keras.layers.Bidirectional(layers.LSTM(hp.Int('layer_size2' + str(i), min_value = 30, max_value = 150, step = 20), return_sequences= num_lstm > i))(x)
        else:
            x = layers.LSTM(hp.Int('layer_size2' + str(i), min_value = 30, max_value = 150, step = 20), return_sequences= num_lstm > i)(x)
