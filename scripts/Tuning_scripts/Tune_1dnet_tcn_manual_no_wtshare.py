...
def temporal_convolutional_net(hp):
    ...
    x = layers.Conv1D(hp.Int("layer_size0", min_value=20, max_value=180, step=20, default = 120), 
                      hp.Int("filter_size0", min_value=5, max_value=60, step=5, default = 32), activation='relu', padding = 'causal',  kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser, 
                      groups = math.gcd(input_shape3d[1], hp.Int("layer_size0", min_value=20, max_value=180, step=20, default = 120)))(input_timeseries)
    x = layers.Conv1D(hp.Int("layer_size1", min_value=20, max_value=170, step=20, default = 80), 
                      hp.Int("filter_size1", min_value=5, max_value=50, step=5, default = 32), activation='relu', padding = 'causal', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser, dilation_rate = 2, 
                      groups = math.gcd(hp.Int("layer_size0", min_value=20, max_value=180, step=20, default = 120), 
                      hp.Int("layer_size1", min_value=20, max_value=170, step=20, default = 80)))(x)
    
    x = layers.MaxPooling1D(hp.Int('pool_size1', 1, 4, 1, default = 1), padding = 'same')(x)

    for i in range(hp.Int('convolutional_blocks', min_value = 0, max_value = 3, default = 1)):
        if i == 1:
            prev_layer = hp.Int("layer_size1", min_value=20, max_value=170, step=20, default = 80)
        else:
            prev_layer = hp.Int("layer_size3" + str((i - 1)), min_value=20, max_value=160, step=20, default = 40)

        x = layers.Conv1D(hp.Int("layer_size2" + str(i), min_value=20, max_value=160, step=20, default = 40), 
                        hp.Int("filter_size2" + str(i), min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'causal', kernel_initializer = init,
                        kernel_regularizer= kernal_regulariser,
                        bias_regularizer= bias_regulariser, 
                        dilation_rate = i*2 + 3, 
                        groups = math.gcd(hp.Int("layer_size2" + str(i), min_value=20, max_value=160, step=20, default = 40), prev_layer))(x)

        x = layers.Conv1D(hp.Int("layer_size3" + str(i), min_value=20, max_value=160, step=20, default = 40), 
                        hp.Int("filter_size3" + str(i), min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'causal', kernel_initializer = init,
                        kernel_regularizer= kernal_regulariser,
                        bias_regularizer= bias_regulariser, dilation_rate = i*2 + 4, 
                        groups = math.gcd(hp.Int("layer_size2" + str(i), min_value=20, max_value=160, step=20, default = 40), 
                                           hp.Int("layer_size3" + str(i), min_value=20, max_value=160, step=20, default = 40)))(x)
...
tuner = MyTuner(
    ...
    project_name="convnet_tcn_log_nowtshare",
)
...
best_model.save('/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_conv_TCN_nowtshare')

#Save the best hyperparameters
best_hyperparameters = tuner.get_best_hyperparameters(1)[0]

a_file = open("/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_conv_TCN_nowtshare.json", "w")
