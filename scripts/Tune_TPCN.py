...
split_transpose = test_trainsplit(np.transpose(outcomes, (0, 2, 1)), np.array([70, 15, 15])) #Produce transpose version so can do pointwise convs
...
def temporal_convolutional_pointwise_net(hp):
    ...
    ####Now make 1d conv net - can probably just leave the convolutional layers to whatver was found doing the 1d convnet
    ...
    x = layers.Conv1D(hp.Int("layer_size2", min_value=20, max_value=160, step=20, default = 40), 
                      hp.Int("filter_size2", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(x)

    x = layers.Conv1D(hp.Int("layer_size3", min_value=20, max_value=160, step=20, default = 40), 
                      hp.Int("filter_size3", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(x)

    x = layers.MaxPooling1D(hp.Int('pool_size2', 1, 4, 1, default = 1), padding = 'same')(x)
    """

    # Now make the pointwise convolutions - will have to scale these so they are appropriate size
    y = layers.Conv1D(hp.Int("layer_size0", min_value=20, max_value=180, step=20, default = 120), 
                      hp.Int("filter_size0", min_value=5, max_value=60, step=5, default = 32), activation='relu', padding = 'same',  kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(input_transpose)
    y = layers.Conv1D(hp.Int("layer_size1", min_value=20, max_value=170, step=20, default = 80), 
                      hp.Int("filter_size1", min_value=5, max_value=50, step=5, default = 32), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(y)
    
    y = layers.MaxPooling1D(hp.Int('pool_size1', 1, 4, 1, default = 1), padding = 'same')(y)

    y = layers.Conv1D(hp.Int("layer_size2", min_value=20, max_value=160, step=20, default = 40), 
                      hp.Int("filter_size2", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(y)

    y = layers.Conv1D(hp.Int("layer_size3", min_value=20, max_value=160, step=20, default = 40), 
                      hp.Int("filter_size3", min_value=5, max_value=40, step=5, default = 16), activation='relu', padding = 'same', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(y)

    y = layers.MaxPooling1D(hp.Int('pool_size2', 1, 4, 1, default = 1), padding = 'same')(y)
...
best_model.save('/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TPCN')

#Save the best hyperparameters
best_hyperparameters = tuner.get_best_hyperparameters(1)[0]

a_file = open("/mhome/damtp/q/dfs28/Project/PICU_project/models/Best_tuned_TPCN.json", "w")
