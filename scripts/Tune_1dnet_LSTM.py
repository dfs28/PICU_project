...
def TCN_lstm(hp):
    ...
    if hp.Choice('bidirectional_init', ['True', 'False']):
        x = tf.keras.layers.Bidirectional(layers.LSTM(hp.Int('layer_size4', min_value = 10, max_value = 150, step = 20), return_sequences = return_sequences))(input_timeseries)
    else:
        x = layers.LSTM(hp.Int('layer_size4', min_value = 10, max_value = 150, step = 20), return_sequences = return_sequences)(input_timeseries)


    ##Now make the other head with input
    y = layers.Dense(hp.Int("layer_size5", min_value=10, max_value=50, step=5, default = 30), activation = 'relu', kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
    ...
    dense1 = layers.Dense(hp.Int("layer_size6", min_value=10, max_value=50, step=5, default = 30), activation = 'relu', use_bias = True, kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(concatted)
    dense2 = layers.Dense(hp.Int("layer_size7", min_value=10, max_value=50, step=5, default = 10), activation = 'relu', use_bias = True, kernel_initializer = init,
                      kernel_regularizer= kernal_regulariser,
                      bias_regularizer= bias_regulariser,
                      activity_regularizer= activity_regulariser)(dense1)

    #Try more layers before multihead and try single head
