...
all_cols = series_cols_mean.copy()
all_cols.extend(series_cols_std)
all_cols.extend(series_cols_slopes)
all_cols.extend(series_cols_R)

series_cols_mean.extend(series_cols_std)

outcome_labels = ['Deterioration >6h', 'No deterioration <6h']
all_inputs = np.concatenate((array2d, characteristics, slopes, r_values), axis=1)
...
all_inputs_cors = np.corrcoef(all_inputs, rowvar=False)
all_inputs_cors[np.isnan(all_inputs_cors)] = 0
for i in range(all_inputs_cors.shape[0]):
    all_inputs_cors[i, i] = np.nan
ax1 = sns.heatmap(all_inputs_cors, xticklabels = all_cols, yticklabels = all_cols)
fig.savefig("/mhome/damtp/q/dfs28/Project/PICU_project/figs/PICU/all_inputs_heatmap.png")

#Plot correlations of point vars and outcomes
fig, (ax1, ax2) = plt.subplots(1, 2, figsize = (40, 20), gridspec_kw={'width_ratios': [1, 2.5]})
array2d_cors_outcomes = np.corrcoef(array2d, binary_deterioration_outcomes, rowvar=False)[53:62, :53]
...
chars_cors_outcomes = np.corrcoef(characteristics, binary_deterioration_outcomes, rowvar=False)[120:, :120]
...
plt.savefig("/mhome/damtp/q/dfs28/Project/PICU_project/figs/PICU/binary_outcomes_heatmap.png")
